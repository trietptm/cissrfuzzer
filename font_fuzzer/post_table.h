...
struct post_table{
       ...
       //pattern of string node: .{1,16}[^0-9a-zA-Z]. 
};
//generating v1 and v3 tables
extern post_table gener_post_table_v1_3(uint32 ver,uint32 Angle, short underlinePos, short underlineThick, uint32 FixedPitch);
extern post_table gener_post_table_v2(post_table pt,uint16 numGlyfs, uint16 new_glyfs_size, uint16* glyphIndex ,char* names);
extern uint32 post_table_size(post_table pt);
extern TableDirectoryNod gener_post_table_header(post_table pt,uint32 offSet);
