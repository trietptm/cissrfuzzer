...
void idaapi mine_pretty_printing(netnode* calls)
{
	...
	char *filename = constrcut_output_filename(".indirect.txt");
	...
	*/
	...
	for(int i = 1; i <= n;i++){
		...
		node->supval(i, &curr_indirect, sizeof(curr_indirect));
		func_t* currFunc = get_func(curr_indirect.caller);
		/*
		if(!currFunc){
			msg("at %d there is no func at 0x08%X\n", i, curr_indirect.caller);
			continue;
		}
		*/
		ua_ana0(curr_indirect.caller);

		//get_nice_colored_name(curr_indirect.caller, arrptr_0, MAXSTR, CNAMEOPT); // address
		qsnprintf(arrptr_0, MAXSTR, "0x%08X", curr_indirect.caller);

		if (curr_indirect.flags & XRSETFLAG)
			qstrncpy(arrptr_1, "x", MAXSTR);
		else
			qstrncpy(arrptr_1, "-", MAXSTR);

		if(currFunc){
			get_short_name(BADADDR, currFunc->startEA, buffer, MAXSTR);
			qsnprintf(arrptr_2, MAXSTR, "%s", buffer);
		}
		else qsnprintf(arrptr_2, MAXSTR, "no function");

		generate_disasm_line(cmd.ea, buffer, sizeof(buffer));
		tag_remove(buffer, buffer, sizeof(buffer));
		qsnprintf(arrptr_3, MAXSTR, "%s", buffer);
