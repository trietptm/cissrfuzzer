/* htmlheadelement_s for new htmlheadelement_s */
/* Create new empty htmlheadelement_s */
function htmlheadelement_() {
	this.name =  "";
	this.all_func = [];
	this.num_func = 0;
	this.all_atrib = [];
...
htmlheadelement_.prototype.add_sleep_object = function(this_type, this_name)
{
	this.dynamic == true;
...
/* Generate "History" */
htmlheadelement_.prototype.generate = function () {
	this.add_attribute ("profile", "DOMString"); 		
...
/* Add name */
htmlheadelement_.prototype.init_name = function (name_parameter) {
	this.name = name_parameter;
};
/* Add attributes */
htmlheadelement_.prototype.add_attribute = function (name_attribute, type_attribute) 
{
	var new_htmlheadelement_ = new attr;
	new_htmlheadelement_.add(name_attribute, type_attribute);
	this.all_atrib.push(new_htmlheadelement_);
};
/* Add attribute-function */
htmlheadelement_.prototype.add_attribute_function = function (name_attribute) 
{
	var new_htmlheadelement_ = new attr_func;
	new_htmlheadelement_.add(name_attribute);
	this.all_atr_func.push(new_htmlheadelement_);
};
/* Add functions(methods) */
htmlheadelement_.prototype.add_functions = function (name_function, fun_type, fun_name)
{
...
/* Add object */
htmlheadelement_.prototype.add_object = function (name_object)
{
	var new_htmlheadelement_ = new htmlheadelement_;
	new_htmlheadelement_.init_name(name_object);
	this.all_object[this.num_object] = new_htmlheadelement_;
	this.num_object ++;
}
htmlheadelement_.prototype.add_object_type = function (type_object, name_object)
{
...
/* Return name of htmlheadelement_s */
htmlheadelement_.prototype.ret_name = function () {
	return this.name;
}
/* Return attributes */
htmlheadelement_.prototype.get_attribute = function () {
	var result = [];
...
/* Return functions */
htmlheadelement_.prototype.get_function = function () {
	var result = [];
...
htmlheadelement_.prototype.return_clas = function (name){
	var return_;
...
htmlheadelement_.prototype.return_i_clas = function (name){
	var return_i;
	for (var j=0; j < this.num_object; j++)
	{
		if ( this.all_object[j].name == name) 
